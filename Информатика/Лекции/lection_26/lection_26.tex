...
\
\setcounter{lec}{25}
\lec{Z--функция строки}
\section{Z--функция строки}
Z--функция строки --- функция от номера символа.
Z--функция --- это массив длинны len(S) = N, z[i] --- длина совпадающего префикса у строки S и S[i:].
...
Зачем она нужна?
Будем искать строчку p=aba и все ее вхождения в строке abacabadabacaba.
s = "aba\#abacabadabacaba"

Т.к. стоит символ \#, длина искомой не может быть больше 3.
z = [0, 0, 1, 0, 3, 0, 1, 0, 3, 0, 1, 0, 3, 0, 1, 0, 3, 0, 1]
Там, где z[i]==len(p), найдена подстрока в строке.
pos = i-len(p)-1 - номер вхождения.
...
\begin{infa}{}
	...
	\num left = right = 0
	\num \fori i \ini \rangei(1, N):
	\num \tab x = min(z[i-left], right - i + 1) if i<=right else 0
	\num \tab \whilei i + x < N \andi s[x] == s[i+x]:
	\num \tab \tab x += 1
	\num \tab z[i] = x
	\num \tab if i + x - 1> right: \com{Сохраянем z--блок}
...
\section{Префикс--функция строки. Алгоритм Алгоритм Кнута — Морриса — Пратта.}
...
"abacaba"
pi = [0,0,1,0,1,2,3]
Эта функция всегда растет на единицу.
\begin{infa}{Тривиальный алгоритм}
\num N = len(s)
\num pi = [0]*N
\num for i in range(1, N):
\num \tab for k in range(i+1):
\num \tab \tab if s[0:k] == s[i-k+1:i+1]:
\num \tab \tab \tab pi[i] = k
\end{infa}
Асимптотика $O(N^3)$.
