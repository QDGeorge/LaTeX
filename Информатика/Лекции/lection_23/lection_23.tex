...
\ \num \classi Tree:
\ \num \tab \classi Node: \com{Класс в классе можно делать, т.к. в классе описано свое пространство имен}
...
\num \tab \tab \whilei p \isi \noti \Nonei \andi p.key != data: \com{Важна последовательность написаний условий, т.к. может быть ошибка при проверке ключа}
...
\num \tab \tab p = self.find(data) \com{Одно и то же число не может храниться дважды, как в множестве, но значения могут повторяться}
\num \tab \tab \ifi p \isi \noti \Nonei:
\num \tab \tab \tab \returni
...
\num \tab \tab \tab \elsei:
\num \tab \tab \tab \tab \ifi p.right \ini \Nonei:
